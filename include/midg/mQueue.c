...
int mQueueCreate(mQueueRH *QRH, mQueueWH *QWH, int mQSize)
  ...
  queueblk *p = (queueblk *) malloc(sizeof(queueblk) + mQSize);
  if(!p)
    return(1);

  //The queue memory has been allocated, so lets initialize the fields.
  p->hw.Callback = 0;
  p->hw.Ptr    = (unsigned char *) (&(p->buf_start));
  p->hw.BufPtr = (unsigned char *) (&(p->buf_start));
  p->hw.EndPtr = (unsigned char *) (&(p->buf_start) + mQSize);

  p->hr.hw  = &(p->hw);
  p->hr.Ptr = (unsigned char *) (&(p->buf_start));
  p->hr.Callback = 0;
